...
40. As a Тренер, I want the Quiz never to ask about a Role decision flagged as unsure, so that I do not memorise something that may be wrong.
41. As a Тренер, I want the rest of an Exercise to stay in the Quiz when only one of its Muscles is flagged, so that 29 Exercises do not disappear because of one doubt each.
41a. As a Тренер, I want the Quiz to show me nothing it is unsure of — not in a question, not among the options, not painted on the map after the answer, not in an explanation — so that the picture I memorise contains only what the author stands behind.
...
- **Пули питань:**
  ...
  - Режим 3 — Вправи, чий Агоніст не позначений як непевний.
  - Режим 4 — усі пари «Вправа + М'яз», крім позначених.
  - Режим 5 — М'язи, що є Агоністом хоча б в одній Вправі, де цю пару не позначено.
- **Непевні рішення** беруться з черги вичитки атласу. Виключається пара «Вправа + М'яз», не Вправа. Непевна пара не буває ні правильною відповіддю, ні відволікаючим варіантом.
- **Непевної пари Гра не показує ніде**, не лише не питає: Розподіл Ролей, який мапа малює після відповіді, легенда й рамка мапи беруться зі списку Ролей, з якого непевні пари вже прибрано. Модуль Гри має один вхід до Ролей Вправи, і питання, варіанти й картина після відповіді йдуть через нього — так правило не можна порушити в окремому Режимі. Довідник (сторінки М'яза й Вправи) цього не стосується: він показує авторський розподіл повністю, доки вичитка не закрита.
...
- **Екран Партії не стрибає.** Шапка з питанням і нижня панель мають сталу висоту — однакову для кожного питання, до відповіді й після неї; мапа займає рівно те, що між ними. Назви вміщаються в свої рамки (довгі — меншим кеглем), а не розсувають їх. Чотири варіанти лишаються на місці й після відповіді (позначені, не прибрані), рядок під ними — спершу підказка, потім вердикт, «Далі» завжди на місці й неактивна до відповіді. Мапа в Партії показує всю фігуру й сама не масштабується; відповідь лише розфарбовує й за потреби повертає фігуру. Розміри залежать лише від екрана пристрою (низький телефон — на щабель менші), не від питання.
...
- **На справжньому контенті**, як тести атласу: контент і є продукт, помилки найімовірніше в ньому. Фікстури — тільки там, де контент треба навмисно зламати (наприклад, Вправа, у якої всі М'язи непевні).
...
- Що перевірити, серед іншого:
  ...
  - жодне питання й жоден варіант не спирається на непевну пару, і жодна непевна пара не потрапляє в те, що Гра малює після відповіді;
...
- **Вичитка непевних рішень** — окрема робота (дослідження черги вичитки й тікет 10), Гра лише їх оминає.
